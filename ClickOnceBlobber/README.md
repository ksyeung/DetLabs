# How do I use this?
Go to https://dataexplorer.azure.com, then create a free ADX cluster account and a new database. Copy the [init.kql](https://github.com/ksyeung/DetLabs/blob/main/fsquirt_Living_off_the_Land/init.kql) data in this folder and paste it into the ADX web UI query window. This will create the required tables, ASIM parsers, helper functions, and start ingestion of the telemetry (data.parquet). If you have an account and database already with content you want to retain, you should create a new database for this lab or **there may be data loss!** The beginning of `init.kql` includes drop commands that clean up data from prior labs, including tables that share names with many common tables.
